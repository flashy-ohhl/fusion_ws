...
# ======================= 配置区域 =======================
# 1. 替换真实文件路径
IMAGE_PATH = "/root/zjj/fusion_ws_2_1/shujuji/1_baseline.jpg"   # 输入图片路径
PCD_PATH = "/root/zjj/fusion_ws_2_1/shujuji/1.pcd"     # 输入点云路径
...
def visualize_detections(image: np.ndarray, detections: List[BoundBox]) -> np.ndarray:
    ...
    for i, det in enumerate(detections):
        ...
        # ⭐ 功能 1：在 2D 图上显示 ID
        label = f"ID:{i} {class_name} {det.score:.2f}"
...
def draw_bev_map(points, boxes_3d_info):
    ...
    # 画框
    for i, box in enumerate(boxes_3d_info):
        ...
        # ⭐ 功能 2：在 BEV 上显示 ID (与 2D 图对应)
        u_text, v_text = lidar_to_bev(cx, cy)
        cv2.putText(bev, f"ID:{i}", (u_text+5, v_text), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
...
class LidarCameraFusion:
    ...
    def fuse_with_pointcloud(self, points, dets):
        ...
        for idx, det in enumerate(dets):
            ...
            # ⭐ 功能 3：保存投影到该 2D 框内的点云 (即使没生成 3D 框也保存)
            if len(roi_pts) > 0:
                pcd_save = o3d.geometry.PointCloud()
                pcd_save.points = o3d.utility.Vector3dVector(roi_pts)
                pcd_filename = os.path.join(PCD_OUTPUT_DIR, f"obj_ID{idx}_roi.pcd")
...
def main():
    ...
    t0 = time.time()
    dets, vis2d = fusion.process_image_array(frame)
    ...
    for i, res in enumerate(results):
        ...
        if res:
            print(f"[ID:{i}] {cname} (score={det.score:.2f}) | 3D Pos: ({res['center_lidar']['x']:.2f}, {res['center_lidar']['y']:.2f}, {res['center_lidar']['z']:.2f})")
        else:
            print(f"[ID:{i}] {cname} (score={det.score:.2f}) | 3D: 未匹配到点云")
